...
**App updates (PWA).** The client ships a service worker (`sw.js`, generated by `apps/client/vite.config.ts` at build time) that caches the app shell, so the app starts fast and solo works offline. `vercel.json` serves `sw.js` with `no-cache` (and `index.html` / the manifest with `no-cache`, hashed `/assets/*` as immutable), so browsers see every deploy. A new deploy installs a new worker that waits: open pages show "Update available — tap to reload". The "New version available — refresh" button also goes through the service worker, so a cached old shell never survives the refresh. The worker only serves the site's own files and never touches the game server's WebSocket.
...
| An installed app (home screen) keeps showing an old version | Tap "Update available — tap to reload". If the banner never shows, check that `/sw.js` is served with `Cache-Control: no-cache` (`vercel.json` headers). |
