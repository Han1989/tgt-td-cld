...
**Player-count scaling:** creep HP × (1 + h × (players − 1)), where h is 1.4 on wave 1 and falls linearly to 0.1 from wave 11 on; creep count +30% per extra player. All numbers are tunable (see Architecture). (Phase 2 used a flat h = 0.5 and +25%; see the Decision Log.)
...
| 2026-09-25 | **Phase 3 balance pass — target:** on Normal, the balance bot wins all 30 waves **with 40–80 Heart HP left**, solo (every hero) and as a 4-bot mixed team (Ranger, Warden, Arcanist, Ranger), on the 5 gate seeds; the idle bot still loses. `balance.test.ts` / `balanceHeroes.test.ts` / `balanceTeam.test.ts` assert it (`HEART_TARGET` in `test/helpers.ts`) | "A challenge, not a walkover" made testable. |
| 2026-09-25 | **Balance bot plays the whole tower kit:** build order Arrow, Frost, Cannon, Arcane, Arrow, Cannon, Flak, Arcane (by its own tower count, so rebuilds and rejected builds don't skew it); once every pad is taken it upgrades its lowest-tier towers, best pads first. Cannon and Arcane use **Strongest**; any tower with a boss in range switches to Strongest until the boss is gone. Heroes attack-move to a live boss instead of their guard post. Teammates start from different free pads so they don't build on the same one | Without upgrades a team hit the 59-pad cap and fell around wave 21. Without Strongest, a boss or archer that stops to hit towers sits behind fresher creeps under First priority: the wave-10 Ironhorn and wave-20 Matriarch lived until the end of the game and ground down tower after tower. |
| 2026-09-25 | **Player-count scaling reshaped:** HP per extra player = `hpPerExtraPlayer` (0.1) + `earlyHpPerExtraPlayer` (1.3) × max(0, 1 − (wave − 1) / `earlyWaves` (10)); creep count +30% per extra player (was a flat +50% HP, +25% count). At 4 players: ×5.2 HP on wave 1, ×1.3 from wave 11. Bosses still aren't multiplied in count | A team has all its gold and heroes from wave 1 but shares one set of 59 pads with a solo player, so its power grows fast early and hits the same cap late. Any flat multiplier was either a walkover early or a collapse late: 4 heroes guarding the Heart hold back a growing backlog until they all die, then 2–3 waves leak at once (Heart 100 → 0 in one wave). Front-loading the pressure makes teams leak gradually in waves 5–11, like a solo player does, and keeps late-game headroom. |
| 2026-09-25 | **Retune:** starting gold 150 → 120; creep HP growth 0.12 → 0.17 per wave; XP share radius 12 → 22 tiles | Solo won with 85–94 Heart and ~3,000 unspent gold once it could upgrade; the extra HP growth gives the late game pressure, and less starting gold makes the first waves bite. Stronger towers took kills far from the heroes, who stopped reaching level 10; the wider XP radius keeps a guarding hero levelling. |
| 2026-09-25 | **Balance results (gate seeds 1, 2, 3, 42, 1234; `npm run balance`):** solo Ranger 61–72, Warden 53–68, Arcanist 59–80; 4 bots 58–79; every idle bot loses (waves 6–8). Over 20 seeds all solo and 4-bot runs win (solo 45–83, 4 bots 44–85). Outside the gate: 2 bots (Ranger + Warden) mostly 54–63 but lose seed 1 on wave 30; 3 bots are easy (68–99) with any hero mix | Records where the numbers stand. 2 and 3 players are not part of the Phase 3 "Done when"; see open questions. |
