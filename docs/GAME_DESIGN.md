...
| 2026-09-25 | **Portrait spike (throwaway, not for merge):** `?map=spire` plays a 45 × 80 "Spire" map (same lane/pad generator as Crossroads) in solo offline mode only; `setActiveMap` picks the map in the worker and client. On a phone held upright the HUD becomes a top status bar + bottom hero bar, with touch controls (tap move/attack, drag a skill to aim, radial build menu, one-finger pan, pinch zoom, Hero button, default zoom fits the map width) | Lets us feel portrait play on a phone before committing to Phase 4 touch controls. |
