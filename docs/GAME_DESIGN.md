...
| 2026-09-25 | **Portrait spike (throwaway, not for merge):** `?map=spire` plays a 31 × 42 "Spire" map (3-tile lanes, 36 pads of 3 × 3; `GameMap.padSize`, 2 on Crossroads) in solo offline mode only; `setActiveMap` picks the map in the worker and client. On a phone held upright (round 3, one-thumb): the whole map is shown between the top bar and a control strip (slim hero row, fixed joystick, Q/W/R arc, E badge; Settings → Layout: one thumb / two thumbs / left-handed). Tap a skill = smart cast (instant skills need a target in reach, point skills hit the densest group in range, Last Stand casts on self; nothing in range shakes the button, no mana spent); drag from it = manual aim, back onto it = cancel. Heroes shoot while moving via an opt-in `GameConfig.moveAndShoot` (off by default, so desktop and online are unchanged). Taps only select (snap within 44 px, picker on ties); radial build menu; entities drawn 1.6× | Lets us feel portrait play on a phone before committing to Phase 4 touch controls; rounds 2–3 follow Android playtests (tap-to-move hit pads; one-thumb play). |
