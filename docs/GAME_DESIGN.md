...
| 2026-09-25 | **Portrait spike (throwaway, not for merge):** `?map=spire` plays a 26 × 56 "Spire" map (2-tile lanes 7 apart, 20 pads of 3 × 3 — `GameMap.padSize`, 2 on Crossroads; Heart at row 33, rows 37+ are forest, `safeFromY`) in solo offline mode only; `setActiveMap` picks the map in the worker and client. On a phone held upright (round 4): the map fills the screen under the top bar, fitted to width (≈15.8 px per tile at 412 px wide); the joystick (35% opacity idle), Q/W/R (52 px, see-through, tight arc) and the E badge are overlaid on the forest rows, and map taps in that bottom 176 px zone are ignored; level, XP and skill points sit in the top bar, HP/mana above the hero. If the playable rows would reach under the controls (short screens) the camera follows the hero vertically. Settings → Layout: one thumb / two thumbs / left-handed. Tap a skill = smart cast (instant skills need a target in reach, point skills hit the densest group in range, Last Stand casts on self; nothing in range shakes the button, no mana spent); drag = manual aim, back onto the button = cancel. Heroes shoot while moving via an opt-in `GameConfig.moveAndShoot` (off by default). Taps only select (snap within 44 px, picker on ties); radial build menu; entities drawn 1.6× | Lets us feel portrait play on a phone before committing to Phase 4 touch controls; rounds 2–4 follow Android playtests. |
