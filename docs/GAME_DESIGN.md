...
| 2026-09-26 | **Match modes are tuning overrides.** `TUNING.modes` holds, per `GameMode` (`full`, `quick`), partial `economy` / `waves` / `playerScaling` / `hero.xpForLevel` sections; `tuningForMode` merges them over the top-level (Full) numbers, and `createGame({ mode })` stores the merged tuning in `state.tuning`, so the engine reads one tuning and never checks the mode. The balance bot merges the same way from `snapshot.mode`. `state.mode` / `snapshot.mode` name the mode | All numbers stay in `tuning.ts`, and a later mode (Phase 4b difficulty) is data, not engine code. |
| 2026-09-26 | **Quick mode protocol:** `LobbyState.mode`, a `{ t: 'mode', mode }` client message (online: host only, lobby only, else `not_host` / `bad_request`; the room keeps its mode after "Back to lobby"; local solo: starts a new match in that mode, like `hero`) and `Snapshot.mode`. A new room starts in Full mode. `PROTOCOL_VERSION` 6 | The host picks the mode in the lobby, solo has its own pick (MOBILE §6). The mode in the snapshot lets the HUD and bots read the right wave list. |
| 2026-09-26 | **Quick mode numbers** (`modes.quick`): 15 waves where wave k is Full wave ~2k (bosses Ironhorn / Matriarch / Shardback on 5 / 10 / 15, air waves on 8 and 13, wave 15 = Full wave 30's list); creep HP growth 0.21 per wave (wave 15 ≈ Full wave 30) and armour growth 0.2; starting gold 120; wave income 84 + 64 × (wave − 1) (about two Full waves' worth); hero XP thresholds × 0.6 (heroes reach level 8–10, solo usually 10 by wave ~12); team HP scaling hp = [1, 1.45, 1.6, 1.8], early bonus [0, 0.5, 1.6, 2.2] fading over 10 waves. Wave interval and build phase unchanged: a match lasts ~11–12 minutes | "Difficulty compressed so wave 15 plays like wave 30". Half the waves means half the leak chances, so each wave is a little meaner than its Full counterpart. With a 10-wave fade, the Full 4-player multipliers left teams losing nothing in waves 11–15; a higher late multiplier (1.8) for 4 players and 1.6 for 3 fixes that without making the final wave a cliff (2.1+ made the Shardback wave decide the match). |
| 2026-09-26 | **Quick mode balance gate** (`balanceQuick.test.ts`, gate seeds): the balance bot wins all 15 waves with 40–80 Heart HP solo with every hero and with 4 players (Ranger, Warden, Arcanist, Ranger), every hero reaches level 8+, and the idle bot loses with every hero on every seed. 2 and 3 players are reported by `npm run balance quick`, not gated | MOBILE §6 asks for 1 and 4 players; solo is gated for every hero like Full mode. |
| 2026-09-26 | **Quick mode balance results** (gate seeds 1, 2, 3, 42, 1234; `npm run balance quick`; Heart HP left, then the average Heart lost in waves 1–5 / 6–10 / 11–15): solo Ranger 44–65 (7/12/24), Warden 46–71 (2/15/20), Arcanist 46–71 (5/12/20); 4 players 50–77 (4/29/0); every idle bot loses at wave 5. Not gated: pairs 33–87, 3 players 44–79 (16/19/0). Over 20 seeds all 160 balance-bot runs win: solo Ranger 23–77 (17 of 20 in range), Warden 44–82 (19), Arcanist 45–78 (20); 2 players 22–91 (49 of 60); 3 players 44–86 (18); 4 players 38–77 (19) | Like Full mode, 3–4-player teams lose almost nothing in the last third because their pads cap their towers. |
| 2026-09-26 | **Client mode pick:** the solo pick shows Full / Quick cards under the hero cards (the last pick is remembered in `localStorage`, Full by default); the online room shows the same cards, clickable for the host and read-only for guests. "Play solo offline" now opens the solo pick instead of starting straight away, and the solo end screen's button reads "Change hero / mode". The HUD's boss banner reads the wave list of the match's mode. MOBILE §11 "Should Quick mode be the default on phones?" is left to track 2 | Solo needs a place to pick the mode; the phone default belongs with the portrait client. |
| 2026-09-26 | Balance bot: a hero with its ultimate plays forward (3+ players) from 40% of the match (Full: wave 12, unchanged; Quick: wave 6) instead of a fixed wave 12. `npm run balance` takes `quick` and its `2p` / `3p` / `4p` filters now work | Wave 12 is most of a Quick match. |
| 2026-09-26 | **Layouts are a pure function** (`apps/client/src/layout.ts`, unit-tested): a **phone** is a touch screen whose short side is under 600 px. A phone held upright gets the **tall** layout; a phone held sideways gets **Rotate to portrait** (read from the screen orientation, not the viewport, so an on-screen keyboard can't trigger it). Anything else gets the **wide** layout (map fitted to the height and centred, HUD in the side margins) when the margins can be ≥ 280 px and tiles ≥ 12 px. Otherwise (portrait tablets, narrow desktop windows) it gets the tall layout, fitted so the whole map shows | MOBILE §4 names phone / tablet / desktop; this makes the switch points exact and testable. |
| 2026-09-26 | **Top bar is 44 px** (touch-target height; the safe-area inset is added above it). **Achieved fit on 412 × 839:** 50 rows × 15.85 px = 792.3 px under the 44 px bar = 836.3 px, 2.7 px to spare, no panning; the controls start 8.9 px below the last gameplay row. Shorter screens (e.g. 390 × 664 iPhone Safari with toolbars, or iPhones with a notch and home indicator in full screen) **follow the hero vertically**, within just the range needed to keep gameplay clear of the controls | Track 1's limit was a top bar ≤ 47 pt. 44 px keeps its buttons at the minimum touch size. |
| 2026-09-26 | **Control geometry:** joystick 100 px across (44 px knob), 52 px Q / W / R buttons and a 34 px E badge on an 84 px arc (one thumb: Q 172°, W 130°, R 50°, E 8° around the joystick, so the cluster is ~150 px tall and fits Spire's 10-row safe zone at 412 px wide). Two thumbs: the joystick in one corner and a quarter arc (Q, W, R, E in the corner) in the other; left-handed mirrors it. On a touch tablet the controls go in the side margins. Stick moves go 2.5 tiles ahead of the hero, resent every 100 ms or on a 0.25 rad turn; release sends `stop` | The spike's round-4 cluster was 176 px tall, more than the safe zone; this arc keeps its button sizes and fits. |
| 2026-09-26 | **Smart cast (answers MOBILE §11):** point skills aim at the **densest group in range** (the creep with the most hittable creeps within the skill's radius), and **ties go to the group nearer the Heart**. The cast point is the group's centre when that is in range, else the seed creep. Ground-only skills (Snare Trap, Meteor, Warden's) ignore flyers. Instant skills cast only with a hittable creep in reach. Warden's Last Stand is a self-buff and always casts. Nothing in range: the button shakes and a toast says "Nothing in range"; nothing is sent | "Densest group" is the rule players can predict; the Heart tie-break covers the close-call case without a second rule. |
| 2026-09-26 | **Map taps:** candidates are your match's pads without a tower, all towers and all creeps. A tap picks the one whose **edge** is nearest, within 44 px; candidates within 6 px of the best are a tie → a picker (up to 4, each with an arrow pointing at it). While a menu is open, a map tap only closes it. A teammate's pad or tower gives a toast ("That pad belongs to Bo", "Bo's Arrow tower") | MOBILE §5's 44 pt snap and "tiny picker" made exact. |
| 2026-09-26 | **Radial menus:** build ring of 5 buttons (58 px) around the pad; the first tap on a tower previews its ghost and range, the second builds. Unaffordable towers are greyed but still previewable. Tower ring: **Upgrade** (top), **Target** (cycles), **Sell** (hold 0.5 s, the button fills). The chip above reads e.g. "Arrow T1: Dmg 16→36 · Spd 1.43→1.54". Menus are clamped on screen and moved up off the controls. Radial menus are used in the tall layout and on any touch screen; desktop mouse in the wide layout keeps the Phase 3 panels | MOBILE §5; desktop behaviour unchanged. |
| 2026-09-26 | Canvas input is split by pointer type: **mouse** → the desktop `Controls`, **touch / pen** → `TouchControls`. Synthesized clicks after a map tap are suppressed (they landed on the radial the tap had just opened) | Keeps both input paths independent on hybrid devices. |
| 2026-09-26 | Creeps and heroes are drawn **× clamp(25 / tile px, 1, 1.6)** (towers at most × 1.2, so they stay inside their pad): ~1.6 on phones, 1 at 25+ px tiles | The spike's playtested 1.6× at ~16 px tiles, without oversizing desktop entities. |
| 2026-09-26 | **Team panel on phones** opens from the Gold stat in the top bar (online only), with the gift buttons | The one-row top bar has no room for a team panel, and gifting is the main way to help a teammate's zone. |
| 2026-09-26 | **Solo pauses when the page is hidden** and shows "Paused — tap to resume" on return. It's a control message to the local worker (`{ ctl: 'pause' }`), not a protocol message, so **track 2 doesn't change `PROTOCOL_VERSION`**. **Online:** when the page comes back, a pending reconnect retries at once, and a socket silent for > 3 s mid-match is dropped and rejoined immediately. A **Screen Wake Lock** is held while a match runs and the page is visible | MOBILE §7. Nothing on the wire changed in track 2. |
| 2026-09-26 | **PWA:** `sw.js` is generated at build time (Vite plugin in `vite.config.ts`). It precaches index.html, every built chunk and the public files; the cache name is a hash of the file list. Navigation is served from the cached shell (fast start, offline solo), and it never touches other origins or the WebSocket. A new worker waits until the player taps "Update available — tap to reload"; the protocol "Refresh" button goes through the same path. `vercel.json` serves `sw.js`, index.html and the manifest with no-cache. Icons (192, 512, maskable 512, apple-touch 180) are generated by `apps/client/scripts/icons.ts`. Android: ⚙ → Install app (the `beforeinstallprompt` event); iPhone: ⚙ → Add to Home Screen opens a short Share-sheet guide | MOBILE §7. |
| 2026-09-26 | **Performance:** device pixel ratio capped at 2; creep and projectile sprites are pooled; creeps off screen are culled. **HP / mana bars are plain sprites, and the Heart and the overlay redraw only when they change:** in Pixi v8, redrawing any small `Graphics` rebuilds the whole scene's draw list, and that rebuild was ~60% of the stress scene's frame cost. Under 4× CPU throttling, JavaScript per frame went from ~27 ms to ~11 ms at 300 creeps. Quality **Auto / High / Low** in ⚙. Low renders at 1× and skips cosmetic effects (splash rings, bounty numbers). Auto starts High and drops to Low for the session when a 3 s window (after a warm-up) averages under 45 FPS | MOBILE §7. |
| 2026-09-26 | **Browser tests** (`npm run test:e2e`, Playwright, Chromium only) run on an `--mode e2e` build that adds a `window.__tdt` debug hook and a `?lab` solo option (5,000 starting gold). Profiles: iPhone 13 (Chromium with the iPhone's viewport, touch and DPR), Pixel 7 (412 × 839), Desktop 1366 × 768. They are separate from `npm test`, which stays Vitest-only. The 300-creep stress test profiles it under **4× CPU throttling** and asserts that the JavaScript per frame (≤ 33 ms) plus the fixed-rate work fits one second at 30 FPS (measured: ~11 ms per frame, ~330 ms of CPU per second). It asserts the frame rate itself (≥ 30 FPS) only on a hardware GPU: on this project's CI-style container (SwiftShader) even a blank full-screen WebGL canvas gets 13 FPS at phone DPR. Real-phone FPS is on the checklist (`?stress=300`, docs/MOBILE_TESTING.md) | Webkit isn't installed here; asserting FPS on a software rasteriser would measure the container, not the game. |
| 2026-09-26 | **Phase 4b effects are client-only and derived, with no protocol change:** a hit is a creep's HP dropping between two rendered snapshots (the client flashes it, throws sparks and sums the damage into a floating number, at most one number per creep every 300 ms; a creep flashes at most every 200 ms so steady fire doesn't bleach its colour); a shot is a projectile appearing next to a tower or hero of its style (recoil, muzzle flash, the Multishot fan); everything else comes from the existing events (`kill`, `crit`, `splash`, `aoe`, `leak`, `waveStart`…). The killing blow's number is the HP the creep had left; a crit's number replaces that creep's normal one. `PROTOCOL_VERSION` stays 6 | A per-hit event would multiply snapshot traffic (hundreds of hits a second at 4 players), and HP deltas already carry the damage. |
| 2026-09-26 | **Effects are pooled particles** (`apps/client/src/render/fx/`): one canvas atlas drawn at start-up (dots, sparks, rings, shards, coins, digits) feeds four `ParticleContainer`s (ground, normal, additive, text) with fixed caps. Damage numbers are one particle per digit, not `Text`. Zones, the Heart, portals and auras are sprites or Graphics drawn once and animated only by transform, alpha and tint; ordinary creeps are drawn white and tinted, so a hit flash is a tint change (bosses keep their colours and flash with an overlay) | In Pixi v8, redrawing a Graphics or toggling `visible` rebuilds the draw list; adding particles or changing a tint doesn't. |
| 2026-09-26 | **Effect levels:** High shows everything. Graphics → **Low** keeps only the *essential* effects (area rings and flashes, hit flashes, recoil, up to 10 damage numbers) and turns off particles (sparks, pops, debris, trails, rain, motes, frost glints, coins) and screen shake. **Screen shake** is its own setting (On by default), under Graphics in ⚙ | MOBILE §7: Low is the performance fallback; shake is also an accessibility preference. |
| 2026-09-26 | **Screen shake** is trauma-based (0..1, decays 1.8/s, offset ∝ trauma², at most 9 px): Meteor 0.8, boss death 0.6, boss wave 0.55, Last Stand 0.35, Stomp 0.3, tower destroyed 0.2, Fireball 0.15, Heart hit 0.12. The Heart itself wobbles and flashes red when hit, beats gently, and at ≤ 30% HP beats faster with a red warning glow (the HUD's Heart stat pulses red too) | Big impacts read as big; small ones stay subtle. |
| 2026-09-26 | **HUD feedback:** gold and Heart numbers count towards their value (exponential chase, always done within 0.6 s); a coin flies from each of your kills to the gold counter (DOM, pool of 12, over the top bar); wave banners show the income, boss waves get a red slam-in banner and a camera shake; every button gets a `pressed` state (set on pointerdown in the capture phase, since touch handlers call `preventDefault`); a skill button flashes when its cast goes through | "Buttons react to presses; gold and Heart count smoothly." |
| 2026-09-26 | **Stress scene exercises effects:** `?stress=N` now hits every creep every tick and streams your kills (coins), splashes, crits, one skill every 1.5 s (Meteor first), leaks, an endless Arrow Storm and a Meteor every 2 s. The perf test forces High quality (Auto could drop to Low mid-run) and asserts particles are live. Measured (Pixel profile, 4× throttling, SwiftShader): 17–19 ms of JavaScript per frame (was ~9 ms), within the 33 ms budget | The ≥ 30 FPS gate must hold with every effect on. |
