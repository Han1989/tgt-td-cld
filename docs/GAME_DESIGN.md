...
| 2026-09-25 | **Portrait spike (throwaway, not for merge):** `?map=spire` plays a 31 × 42 "Spire" map (3-tile lanes, 36 pads of 3 × 3; `GameMap.padSize`, 2 on Crossroads) in solo offline mode only; `setActiveMap` picks the map in the worker and client. On a phone held upright: top status bar, one slim hero row + big Q/W/E/R; drag anywhere = floating joystick (release = stop, so the idle hero auto-attacks), taps only select (snap within 44 px to pad / own tower / creep, a small picker on ties), drag a skill to aim, radial build menu, entities drawn 1.6×, camera fits the map width and follows the hero vertically only if the map doesn't fit | Lets us feel portrait play on a phone before committing to Phase 4 touch controls. Round 2 after playtesting on Android: tap-to-move kept hitting pads. |
