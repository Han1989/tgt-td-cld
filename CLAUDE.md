...
| `npm run balance [seeds…]` | Print solo balance-bot and idle-bot results for every hero, plus mixed teams of 2, 3 and 4 bots; use it after editing `tuning.ts` |
...
packages/sim/              @tdt/sim: pure deterministic simulation
  ...
  src/bots.ts              Balance bot (builds, upgrades, focuses bosses) and idle bot; commands only
...
- **Balance gate:** `balance.test.ts` requires the balance bot to win all 30 waves solo **with 40–80 Heart HP left** (`HEART_TARGET` in `test/helpers.ts`) and the idle bot to lose on 5 seeds; `balanceHeroes.test.ts` does the same for the Warden and the Arcanist, and `balanceTeam.test.ts` for a 4-bot mixed team (~40 s, the slowest file). The files run in parallel. Results swing ±15–20 Heart HP between seeds, so check a wider seed list (`npm run balance 1 2 3 … 19`) before trusting a change. After changing `tuning.ts`, run `npm run balance` and keep every outcome true.
