...
| `npm run balance [quick] [solo\|teams\|2p\|3p\|4p] [seeds…]` | Print solo balance-bot and idle-bot results for every hero, plus mixed teams (three 2-bot pairs, 3 and 4 bots), with Heart HP lost per third of the match, towers branched and each player's unspent gold at the end, in Full mode (or Quick with `quick`); use it after editing `tuning.ts` or `bots.ts` |
...
packages/sim/              @tdt/sim: pure deterministic simulation
  ...
  src/towers.ts            Tower targeting (First/Strongest/Closest), upgrades and branches (tier 4: Volley, Blizzard pulse, Prism chains,
                           Glacier freeze, Shrapnel shred, Void pierce…), projectiles, Snare Traps
  ...
  src/bots.ts              Balance bot (own-zone pads, full tower kit from the wave list, upgrades, branches from the wave list and the team's
                           branches, gifts its gold once it has nothing left to buy; a hero that only walks) and idle bot
...
apps/client/               @tdt/client: Vite + PixiJS + HTML/CSS HUD
  ...
  src/hud/                 DOM HUD, desktop tower panels, end screen, settingsPanel.ts; towerInfo.ts = tower and branch stat text (DOM-free, tested);
...
- Towers: per-tier numbers live in `TUNING.towers[kind].tiers` (tier 1 = build); read them with `towerTier(tuning, kind, tier)`. After the last tier a tower upgrades into one of its two branches (`TOWER_BRANCHES` in the protocol, numbers in `TUNING.branches`): tier 4, `tower.branch` set, final. The `upgrade` command carries `branch` exactly then (the sim rejects it before, and rejects a plain upgrade at tier 3). Engine code reads a tower's numbers with `towerStats(tuning, kind, tier, branch)`, which merges the branch's effects (`TowerEffects`) over plain defaults (`NO_EFFECTS`); plain-tier shots carry no `fx`, so they behave exactly as before branches. A new branch effect needs a `TowerEffects` field, handling in `towers.ts`, a stat row in the client's `towerInfo.ts` and a test in `branches.test.ts`.
