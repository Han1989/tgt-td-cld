...
| `npm run test:e2e` | Playwright browser tests (`apps/client/e2e`): builds `apps/client/dist-e2e` with `--mode e2e`, then runs portrait iPhone / Pixel emulation, desktop, PWA and, alone at the end, the 300-creep stress test in Chromium (~3.5 min); `?lab&auras` starts heroes with E learned |
...
apps/client/               @tdt/client: Vite + PixiJS + HTML/CSS HUD
  ...
  playwright.config.ts     Projects: iphone, pixel (Chromium mobile emulation), desktop; perf (the stress test, run alone after the rest)
